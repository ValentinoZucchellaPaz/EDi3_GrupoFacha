...
// Variable global del duty cycle actual
volatile uint32_t duty = 0;
volatile uint8_t fade_in = 1;    // 1 = fade in, 0 = fade out
...
void TIMER0_IRQHandler(void) {
    ...
    if (LPC_TIM0->IR & (1 << 1)) {
        // limpio flag, prendo led
        LPC_TIM0->IR = (1 << 1);
        if (duty > 0)
            LPC_GPIO0->FIOCLR = (1 << 22);

        // cuento 10 MR1 (10ms) entonces actualizo duty cycle segun fade in/out
